...
//$type=$_REQUEST["type"];
  ...
  if ($ID) {
    ...
    $wpdb->update(
            $dd_table, array('eliminato' => 0), array('fk_dokumente' => $ID), array('%d')
    );
  ...
  <form name="form_insert" action="" method="post" id="form_insert">
      ...
      <div id="tabstrip">
          ...
          <div>
              ...
              <div class="customer_data_partial" data-customer="<?php if (isset($fk_clinti)) echo $fk_kunde ?>">
                  <input type="hidden" id="tipo_cliente" name="tipo_cliente" value="<?php if (isset($tipo_cliente)) echo $tipo_cliente ?>" data-value="<?php if (isset($tipo_cliente)) echo $tipo_cliente ?>" />                  <div class="row form-group">
                      ...
                      <div class="col-sm-3">
                          ...
                          <input type="hidden" name="hidden_fk_kunde" value="<?php if (isset($fk_kunde)) echo $fk_kunde ?>">
  ...
  <ul class="select-action">
      ...
      if ($ID) {
        if (WPsCRM_advanced_print()) {
          ?>
          <li class="btn btn-sm btn-info _flat" onclick="location.replace('?page=smart-crm&p=dokumente/advanced_print.php&id_invoice=<?php echo $ID ?>')">
...
jQuery(document).ready(function ($) {
    ...
    <?php if (isset($fk_kunde) && $fk_kunde) : ?>
        $("#fk_kunde").append(new Option("<?php echo esc_js($cliente); ?>", "<?php echo esc_js($fk_kunde); ?>", true, true)).trigger('change');
    ...
    $('#_submit').on('click', function (e) {
        aggiornatot();
        var valid = true;
        if ($("#fk_kunde").val() == "" || $("#fk_kunde").val() == null) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.indexOf('OK') != -1) {
                    ...
                    <?php } else if (!$ID) { ?>
                        setTimeout(function () {
                            location.href = "<?php echo admin_url('admin.php?page=smart-crm&p=dokumente/form_quotation.php&ID=') ?>" + id_cli;
                        }, 1000);
                    <?php } ?>
                } else {
                    noty({
    ...
    window.annulla = function () {
        location.href = "<?php echo admin_url('admin.php?page=smart-crm&p=dokumente/list.php') ?>";
    };
});
</script>
  <?php
