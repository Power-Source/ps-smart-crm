...
//echo $_POST["totale"];
	...
	$fk_kunde=isset($_POST["fk_kunde"])?$_POST["fk_kunde"]:0;
	$hidden_fk_kunde=isset($_POST["hidden_fk_kunde"])?$_POST["hidden_fk_kunde"]:0;
	if ($ID=$_GET["ID"])
	{
		...
		$result=$wpdb->query(
			$wpdb->prepare(
				"delete $s_table.* from $s_table inner join $a_table on $s_table.ID= $a_table.fk_subscriptionrules where s_specific<>0 and $a_table.fk_dokumente=%d",
				$ID
				)
			);
	...
	while (true)
	{
		...
		if ($id_art)
		{
			...
			if ($idd)
			{
				$wpdb->update(
			  $d_table,
			  array('fk_dokumente' => $ID_ret,'fk_artikel' => $id_art, 'prezzo' => $prezzo, 'sconto' => $sconto, 'qta' => $qta, 'totale' => $totale, 'n_riga' => $i, 'codice' => $codice, 'descrizione' => $descrizione, 'iva' => $iva),
			array('id'=>$idd),
			...
			else
			{
				$wpdb->insert(
			  $d_table,
			  array('fk_dokumente' => $ID_ret,'fk_artikel' => $id_art, 'prezzo' => $prezzo, 'sconto' => $sconto, 'qta' => $qta, 'totale' => $totale, 'n_riga' => $i, 'codice' => $codice, 'descrizione' => $descrizione, 'iva' => $iva, 'tipo' => $tipo, 'fk_subscriptionrules' => $subscriptionrules),
...
//		var_dump( $wpdb->last_query );
		...
		else
		{
			...
			if ($res)
			{
				$wpdb->update(
			  $d_table,
			  array('fk_dokumente' => $ID_ret, 'prezzo' => $prezzo, 'sconto' => $sconto, 'qta' => $qta, 'totale' => $totale, 'n_riga' => $i, 'codice' => $codice, 'descrizione' => $descrizione, 'iva' => $iva),
			array('id'=>$idd),
			...
			else
			{
				$wpdb->insert(
			  $d_table,
			  array('fk_dokumente' => $ID_ret, 'prezzo' => $prezzo, 'sconto' => $sconto, 'qta' => $qta, 'totale' => $totale, 'n_riga' => $i, 'codice' => $codice, 'descrizione' => $descrizione, 'iva' => $iva, 'tipo' => $tipo, 'fk_subscriptionrules' => $subscriptionrules),
...
if ($type==1)
{
    header("location: ".admin_url('admin.php?page=smart-crm&p=dokumente/form_quotation.php')."&ID=$ID_ret&type=$type&saved=1");
}
elseif ($type==2)
{
    header("location: ".admin_url('admin.php?page=smart-crm&p=dokumente/form_invoice.php')."&ID=$ID_ret&type=$type&saved=1");
}
elseif ($type==3)
{
    header("location: ".admin_url('admin.php?page=smart-crm&p=dokumente/form_invoice_informal.php')."&ID=$ID_ret&type=$type&saved=1");
}
else
{
    header("location: ".admin_url('admin.php?page=smart-crm&p=dokumente/form_credit_note.php')."&ID=$ID_ret&type=$type&saved=1");
}
exit;
?>
