...
$sql = "select fk_artikel, n_riga, prezzo, totale, qta, codice, descrizione, sconto, tipo, iva from $dd_table where fk_dokumente=$ID order by $dd_table.n_riga";
$qd = $wpdb->get_results($sql, ARRAY_A);
if ($qd) {
  $_header = "";
  //check if records with code/discount exists: if not, I will hide the columns
  $count_c = $wpdb->get_var("SELECT COUNT(*) FROM $dd_table  where fk_dokumente=$ID and codice<>''");
  $count_d = $wpdb->get_var("SELECT COUNT(*) FROM $dd_table  where fk_dokumente=$ID and sconto<>0");
