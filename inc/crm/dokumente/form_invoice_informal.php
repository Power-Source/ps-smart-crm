...
else
{
    ...
    /*    if ($riga["FK_contatti"])
    	...
    	$wpdb->update(
    	  $dd_table,
    	  array('eliminato' => 0),
    	array('fk_dokumente'=>$ID),
    	  array('%d')
    	);
        $sql="select * from $dd_table where fk_dokumente=$ID order by n_riga";
        $qd=$wpdb->get_results( $sql, ARRAY_A);
        $sql="select $s_table.* from $s_table, $a_table where fk_dokumente=$ID and $s_table.ID =$a_table.fk_subscriptionrules and $a_table.fk_dokumente_dettaglio=0";
...
<form name="form_insert" action="" method="post" id="form_insert">
    ...
    <h1 style="text-align:center;background-color:#ffd7d7"><?php _e('INFORMELLE RECHNUNG ERSTELLEN/BEARBEITEN','cpsmartcrm')?> <i class="glyphicon glyphicon-list-alt"></i></h1>
    <div id="tabstrip">
        <ul>
        ...
        <div>
            <h4 class="page-header" style="margin: 10px 0 20px;">
                <?php _e('INVOICE DATA','cpsmartcrm')?><span class="crmHelp" data-help="document-data"></span>
                <?php if ($ID) {?>
            ...
            <div class="row form-group">
                ...
                <label class="control-label"><?php _e('Zahlungsexp. Datum','cpsmartcrm')?></label>
                <div class="col-sm-2">
                    <input name="data_scadenza" id="data_scadenza" class="_m" data-placement="bottom"  value="<?php if (isset($riga)) echo  $data_scadenza  ?>"/>
                </div>
                <?php
                if ($ID)
                {
            ...
            <div class="customer_data_partial" data-customer="<?php if (isset($fk_kunde)) echo $fk_kunde?>">
                ...
                <div class="row form-group">
                    ...
                    <div class="col-sm-3">
						...
						$disabled="disabled readonly";
							...
							$disabled="";
						?>
                        <select id="fk_kunde" name="fk_kunde" data-parsley-hasclient></select>
						<input type="hidden" name="hidden_fk_kunde" id="hidden_fk_kunde" value="<?php if (isset($fk_kunde)) echo $fk_kunde?>">
                ...
                <div class="row form-group">
                    ...
                    <div class="col-sm-2">
                        ...
                        <input type="text" class="form-control _editable" name="provinz" id="provinz" maxlength='5' value="<?php if (isset($provinz)) echo $provinz?>" <?php echo $disabled?> data-value="<?php if (isset($provinz)) echo $provinz?>">
	...
	if ($ID)
	{
        ...
        ?>
        <li class="btn btn-sm btn-info _flat" onclick="location.replace('?page=smart-crm&p=dokumente/document_print.php&id_invoice=<?php echo $ID?>')">
...
<div id="dialog_todo" style="display:none;" data-from="dokumente" data-fkcliente="<?php echo $fk_kunde?>">
	<?php include ( WPsCRM_DIR."/inc/crm/kunde/form_todo.php" ) ?>
</div>
<div id="dialog_appuntamento" style="display:none;" data-from="dokumente" data-fkcliente="<?php echo $fk_kunde?>">
	<?php include (WPsCRM_DIR."/inc/crm/kunde/form_appuntamento.php" ) ?>
</div>
<div id="dialog_attivita" style="display:none;"  data-from="dokumente" data-fkcliente="<?php echo $fk_kunde?>">
	<?php include (WPsCRM_DIR."/inc/crm/kunde/form_attivita.php" ) ?>
</div>
<div id="dialog_mail" style="display:none;" data-from="dokumente" data-fkcliente="<?php echo $fk_kunde?>">
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                if (response.indexOf('OK') != -1) {
                    ...
                    $("#ID").val(id_cli);
                    setTimeout(function () {
                        location.href = "<?php echo admin_url('admin.php?page=smart-crm&p=dokumente/form_invoice_informal.php&ID=') ?>" + id_cli;
                    }, 1000)
                } else {
                    noty({
